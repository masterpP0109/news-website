...
input double   FVG_Entry_Buffer_Pct    = 10.0; // Now in percentage (10%)
input double   FVG_Min_Gap_ATR         = 0.5;  // Min gap size in ATR multiples
...
input double   SR_Padding_ATR         = 0.5;  // ATR-based padding for S/R levels
input double   SR_Confluence_ATR_Mult = 1.2;  // Added: ATR multiplier for S/R confluence

input group "Notifications"
input bool     EnablePushNotifications = true;
input bool     EnableChartArrows       = true;
input int      Signal_Detail_Level    = 2;
input int      Max_Arrows_To_Keep     = 50;   // Prevent chart clutter
...
input int      Max_Trades_Per_Symbol   = 1;    // Prevent over-trading
input int      Min_Bars_Between_Trades = 3;    // Bar count between trades
input int      Max_Slippage            = 3;    // Added: Max slippage in points

input group "Debug"
input int      DebugModeLevel          = 0;    // Added: 0=all, 1=no plot, 2=no notifications
...
// SuperTrend history arrays (now 2D: symbolIndex x timeframe)
double superTrendValues[][3]; // [symbolIndex*tfCount][0=value, 1=trend, 2=prevValue]
...
SCachedIndicators cachedIndicators[][5]; // [symbolIndex][tfIndex]
...
int OnInit()
{
    ...
    ArrayResize(cachedIndicators, symbolCount);
    
    for(int i = 0; i < symbolCount; i++)
    {
        SymbolsArray[i] = StringTrim(SymbolsArray[i]);
        
        // Initialize cached indicators array for this symbol
        ArrayResize(cachedIndicators[i], tfCount);
...
SIndicators GetIndicators(const string sym, const ENUM_TIMEFRAMES tf, const int symbolIndex, const int tfIndex)
{
    ...
    if(cachedIndicators[symbolIndex][tfIndex].lastBarTime == currentBarTime)
    {
        // Return cached values
        indicators.atr = cachedIndicators[symbolIndex][tfIndex].atr;
        indicators.stValue = cachedIndicators[symbolIndex][tfIndex].stValue;
        indicators.ema50 = cachedIndicators[symbolIndex][tfIndex].ema50;
        indicators.adx = cachedIndicators[symbolIndex][tfIndex].adx;
        indicators.donchianHigh = cachedIndicators[symbolIndex][tfIndex].donchianHigh;
        indicators.donchianLow = cachedIndicators[symbolIndex][tfIndex].donchianLow;
    ...
    // Get ATR first
    indicators.atr = iATR(sym, tf, SuperTrend_ATRPeriod, 1);
    ...
    int stIndex = symbolIndex * ArraySize(Timeframes) + tfIndex;
    double previousST = (superTrendValues[stIndex][0] != 0) ? 
                        superTrendValues[stIndex][0] : finalLower;
    int previousTrend = (superTrendValues[stIndex][1] != 0) ? 
    ...
    // EMA
    indicators.ema50 = iMA(sym, tf, EMA50_Period, 0, MODE_EMA, PRICE_CLOSE, 1);
    
    // ADX
    indicators.adx = iADX(sym, tf, ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
    
    // Donchian Channel
    indicators.donchianHigh = iHigh(sym, tf, iHighest(sym, tf, MODE_HIGH, Donchian_Period, 1));
    indicators.donchianLow = iLow(sym, tf, iLowest(sym, tf, MODE_LOW, Donchian_Period, 1));
    
    // Cache the values
    cachedIndicators[symbolIndex][tfIndex].lastBarTime = currentBarTime;
    cachedIndicators[symbolIndex][tfIndex].atr = indicators.atr;
    cachedIndicators[symbolIndex][tfIndex].stValue = indicators.stValue;
    cachedIndicators[symbolIndex][tfIndex].ema50 = indicators.ema50;
    cachedIndicators[symbolIndex][tfIndex].adx = indicators.adx;
    cachedIndicators[symbolIndex][tfIndex].donchianHigh = indicators.donchianHigh;
    cachedIndicators[symbolIndex][tfIndex].donchianLow = indicators.donchianLow;
                       ...
                       double &fvgLow, double &fvgHigh, bool &isBull, int &foundShift)
{
    ...
    double atr = iATR(sym, tf, SuperTrend_ATRPeriod, 1);
    ...
    if(tickValue <= 0 || point <= 0) 
    {
        ...
        if(contractSize > 0 && tickSize > 0)
        {
            ...
            else
            {
                ...
                tickValue = tickSize * contractSize * MarketInfo(sym, MODE_TICKVALUE) / point;
            }
        }
        else
        {
...
void CleanOldArrows(const string sym)
{
    ...
    ArraySort(oldestTimes, 0, 0, MODE_ASCEND);
...
void OnTick()
{
    ...
    for(int s = 0; s < ArraySize(SymbolsArray); s++)
    {
        ...
        if(Bars(sym, PERIOD_CURRENT) < 50) continue;
        ...
        for(int tfIdx = 0; tfIdx < tfCount; tfIdx++)
        {
            ...
            if(lastSignalTime[signalIndex] == currentTime) continue;
